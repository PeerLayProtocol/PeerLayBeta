...
function createUserPrediction(predictionID, gameID, combinationID, score, prize, redeem, isRedeemable ) {
    return {
...
async function fetchPredictionsBatch(indices) {
    try {
        ...
        predictionDetailsArray.forEach((prediction, i) => {
            ...
            const game = Number(prediction[1]) || 0; // Validate number
            const alreadyRedeemed = Number(prediction[2]) || 0;
            ...
            if (game <= lastGameCalculated) {
                const totalPrize = BigInt(prediction[4]) || 0n; // Validate BigInt
                const totalScores = BigInt(prediction[5]) || 0n; // Validate BigInt
                score = BigInt(prediction[3]) || 0n; // Validate BigInt
                const contributorsPrize = Number(prediction[6]) || 0;

                const rawPrize = (score * totalPrize) / totalScores;
...
function updateModalText(modalTextElement, prediction) {
    modalTextElement.innerHTML = `
        <p><strong>Score:</strong> ${prediction.score}</p>
    `;
}
